...
import type { Doctor, AppointmentWithDetails, Patient, AppointmentStatus, AppointmentType } from '../../types';
...
export default function DoctorAgenda() {
  ...
  const load = useCallback(async () => {
    ...
    setDoctor(doc);
    if (doc) {
      const [appts, pats] = await Promise.all([fetchAppointmentsByDoctor(doc.id), fetchPatients()]);
            ...
            <div key={apt.id} className="card p-4 hover:shadow-md transition-shadow">
              <div className="flex items-start justify-between gap-3 flex-wrap">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  <select
                    className="text-xs rounded-lg border border-slate-200 px-2 py-1.5 text-slate-600 focus:outline-none focus:border-blue-500"
                    value={apt.status}
